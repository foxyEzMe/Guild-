                ...
                >
                  ...
                  <AnimatePresence>
                    {hoveredItem === item.path && (
                      <motion.div
                        className="absolute inset-0 -z-10 rounded-md opacity-30"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 0.15 }}
                        exit={{ opacity: 0 }}
                        style={{
                          background: `linear-gradient(120deg, ${item.color}60, transparent)`
                        }}
                      />
                    )}
                  </AnimatePresence>
              ...
              {isAuthenticated ? (
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="w-56 backdrop-blur-xl bg-black/80 border border-gray-700 text-white p-2 rounded-xl">
                    <DropdownMenuItem className="flex items-center space-x-2 px-3 py-2 rounded-md cursor-pointer hover:bg-white/10" onClick={() => navigateTo('/profile')}>
                      <User className="w-4 h-4" />
                      <span>Mon profil</span>
                    </DropdownMenuItem>
                    <DropdownMenuItem className="flex items-center space-x-2 px-3 py-2 rounded-md cursor-pointer hover:bg-white/10" onClick={() => navigateTo('/settings')}>
                      <Settings className="w-4 h-4" />
                      <span>Paramètres</span>
                    </DropdownMenuItem>
                    {user?.isAdmin && (
                      <DropdownMenuItem className="flex items-center space-x-2 px-3 py-2 rounded-md cursor-pointer hover:bg-white/10" onClick={() => navigateTo('/admin')}>
